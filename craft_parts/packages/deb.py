    ...
    try:
        output = (
            subprocess.check_output(
                ["dpkg-query", "-S", str(Path(os.path.sep) / file_path)],
...
class Ubuntu(BaseRepository):
    ...
    def _unpack_stage_debs(
        ...
        *,
        ...
        for _pkg_path in stage_packages_path.glob("*.deb"):
            pkg_path = Path(_pkg_path)
            with tempfile.TemporaryDirectory(
                suffix="deb-extract", dir=install_path.parent
            ) as _extract_dir:
                extract_dir = Path(_extract_dir)
                # Extract deb package.
                deb_utils.extract_deb(pkg_path, extract_dir, logger.debug)
                ...
                file_utils.link_or_copy_tree(extract_dir, install_path)
