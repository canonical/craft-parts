...
class PartHandler:
    ...
    def _create_usrmerge_scaffolding(self, *install_dirs: Path) -> None:
        ...
        if len(install_dirs) != 1:
            # Partitions enabled; bypass for now (we need to define what to do)
            return

        merge_to_usr = ("bin", "lib", "lib64", "sbin")

        for root_dir in install_dirs:
            for dir_name in merge_to_usr:
                ...
                if (
                    ...
                    # Link already exists, probably from a previous run that
                    # wasn't cleaned.  If some of the above are true (like it's
                    # a link to a different path, or a dir) then let the
                    # symlink_to call below fail.
                    continue
    ...
    def _make_dirs(self) -> None:
        ...
        self._create_usrmerge_scaffolding(*self._part.part_install_dirs.values())
