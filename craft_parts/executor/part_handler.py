...
class PartHandler:
    ...
    def _run_build(
        ...
        *,
        ...
        with _conditional_layer_mount(
            self._overlay_manager, top_part=self._part, condition=condition
        ):
            self._run_step(
                step_info=step_info,
    ...
    def _fetch_overlay_packages(self) -> None:
        ...
        logger.debug("fetch overlay packages: %s", overlay_packages)

        try:
            # Parts declaring overlay packages are ordered to be processed after
            # parts organizing to the overlay, which should prepare the environment
