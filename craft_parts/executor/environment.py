...
def _basic_environment_for_part(part: Part, *, step_info: StepInfo) -> dict[str, str]:
    ...
    if include_paths:
        for envvar in ["CPPFLAGS", "CFLAGS", "CXXFLAGS"]:
            part_environment[envvar] = f"${{{envvar}:-}} " + _combine_paths(
    ...
    if library_paths:
        part_environment["LDFLAGS"] = "${LDFLAGS:-} " + _combine_paths(
    ...
    if pkg_config_paths:
        part_environment["PKG_CONFIG_PATH"] = "${PKG_CONFIG_PATH:-} " + _combine_paths(
