...
def _get_contents(source: pathlib.Path) -> tuple[set[str], set[str]]:
...
def _get_candidates_from_overlay(
    ...
    for i, part in enumerate(parts_with_overlay):
        ...
        files, dirs = _get_contents(part_layer_dir)
...
def _check_for_stage_collisions_per_partition(
    ...
    for part_ in part_list:
        ...
        # Scan previous parts for collisions. Since ``all_candidates`` contains candidates
        # from the overlay, this will also check for collisions between install dirs and
        # layers.
