...
it performs some or all of the steps described in the :ref:`parts lifecycle
...
The source for a part is described using the :ref:`reference-part-properties-source`
property. This specifies a location where the source code or other information is to be
*pulled* from. This may be a repository on a remote server, a directory on the build
host, or some other location.
...
:ref:`reference-part-properties-source-type` property is used to explicitly specify the
source format. This influences the way in which the source code or data is processed. A
list of supported formats can be found in the :mod:`craft_parts.sources` file. These
include repository types, such as ``git``, archive formats such as ``zip``, and
``local`` for files in the local file system.

If the source type represents a file, the
:ref:`reference-part-properties-source-checksum` property can be used to provide a
checksum value to be compared against the checksum of the downloaded file.

Parts with source types that describe repositories can also use additional properties to
accurately specify where source code is found. The
:ref:`reference-part-properties-source-branch`,
:ref:`reference-part-properties-source-commit` and
:ref:`reference-part-properties-source-tag` properties allow sources to be obtained from
a specific branch, commit or tag.

Since some repositories can contain large amounts of data, the
:ref:`reference-part-properties-source-depth` property can be used to specify the number
of commits in a repository's history that should be fetched instead of the complete
history. For repositories that use submodules, the
:ref:`reference-part-properties-source-submodules` property can be used to fetch only
those submodules that are needed.

The :ref:`reference-part-properties-source-subdir` property specifies the subdirectory
in the unpacked sources where builds will occur. **Note:** This property restricts the
build to the subdirectory specified, preventing access to files in the parent directory
...
The dependencies of a part are described using the
:ref:`reference-part-properties-build-snaps` and
:ref:`reference-part-properties-build-packages` properties. These specify lists of snaps
and system packages to be installed before the part is built. If a part depends on other
parts, the :ref:`reference-part-properties-after` property is used to specify these --
see :ref:`parts_build-order`.
...
``libsdl2-dev`` package in the :ref:`reference-part-properties-build-packages` property.
...
to manually specify the build actions to be taken, using the
:ref:`reference-part-properties-override-build` property. This property can also be used
to replace or extend the build process provided by a plugin.
...
The :ref:`reference-part-properties-build-environment` property defines assignments to
...
The :ref:`reference-part-properties-organize` property is used to customise how files
are copied from the building area to the staging area. It defines an ordered dictionary
that maps paths in the building area to paths in the staging area.
...
to be deployed with the part are specified using the
:ref:`reference-part-properties-stage-snaps` and
:ref:`reference-part-properties-stage-packages` properties. Files to be deployed are
specified using the :ref:`reference-part-properties-stage` property.

.. ifconfig:: project in ("Rockcraft", "Snapcraft")

   Chisel slices can be specified in :ref:`reference-part-properties-stage-packages` as
   well, but they cannot be mixed with deb packages. 
...
:ref:`reference-part-properties-after` property to define this relationship. This
property specifies a list containing the names of parts that it will be built after. The
parts in the list will be *built and staged* before the part is built.
