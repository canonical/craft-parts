...
def test_get_build_commands(plugin, new_dir):
    assert plugin.get_build_commands() == [
        f'"${{HOME}}"/.cargo/bin/uv venv ${{PARTS_PYTHON_VENV_ARGS}} "{new_dir}/parts/p1/install"',
        f'PARTS_PYTHON_VENV_INTERP_PATH="{new_dir}/parts/p1/install/bin/${{PARTS_PYTHON_INTERPRETER}}"',
        f"VIRTUAL_ENV={new_dir}/parts/p1/install",
        f'"${{HOME}}"/.cargo/bin/uv pip install  -U pip setuptools wheel',
        f'[ -f setup.py ] || [ -f pyproject.toml ] && "${{HOME}}"/.cargo/bin/uv pip install  -U .',
...
def test_call_should_remove_symlinks(plugin, new_dir, mocker):
    mocker.patch(
        "craft_parts.plugins.uv_plugin.UvPlugin._should_remove_symlinks",
        return_value=True,
    )
