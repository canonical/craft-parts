...
def test_generate_step_environment_build(
    ...
    assert env == textwrap.dedent(
        ...
        export CPPFLAGS="${{CPPFLAGS:-}} -isystem {new_dir}/parts/p1/install/usr/include"
        export CFLAGS="${{CFLAGS:-}} -isystem {new_dir}/parts/p1/install/usr/include"
        export CXXFLAGS="${{CXXFLAGS:-}} -isystem {new_dir}/parts/p1/install/usr/include"
        export LDFLAGS="${{LDFLAGS:-}} -L{new_dir}/stage/lib -L{new_dir}/stage/usr/lib -L{new_dir}/stage/lib/aarch64-linux-gnu"
        export PKG_CONFIG_PATH="${{PKG_CONFIG_PATH:-}} {new_dir}/stage/usr/share/pkgconfig"
        ## Plugin environment
        ## User environment
