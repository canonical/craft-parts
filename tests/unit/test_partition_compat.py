...
class TestProjectInfoCompat:
    ...
    def test_default_filesystem_mount_uses_aliased_default_partition(self, new_dir):
        info = ProjectInfo(
            application_name="test",
            cache_dir=new_dir,
            partitions=["mypart", "yourpart"],
        )
        assert info.default_filesystem_mount == FilesystemMount(
            root=[FilesystemMountItem(mount="/", device="mypart")]
        )
