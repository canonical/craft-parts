...
class TestPartHandling:
    ...
    def test_run_pull(self, mocker):
        ...
        state = self._handler._run_pull(
            StepInfo(self._part_info, Step.PULL),
            stdout=None,
            stderr=None,
        )
    ...
    def test_run_build_out_of_source_behavior(self, mocker, out_of_source):
        ...
        pytest_check.is_not(
            (self._part_info.part_build_dir / "source.c").exists(),
            out_of_source,
        )
    ...
    def test_run_prime(self, mocker):
        ...
        state = handler._run_prime(
            StepInfo(self._part_info, Step.PRIME),
            stdout=None,
            stderr=None,
        )
...
class TestPartUpdateHandler:
    ...
    def test_update_build_stage_packages(self, partitions, mocker):
        ...
        mocker.patch(
            "craft_parts.packages.Repository.unpack_stage_packages",
            new=fake_unpack,
        )
        mocker.patch(
            "craft_parts.packages.Repository.fetch_stage_packages",
...
class TestPackages:
    ...
    def test_fetch_stage_packages(self, mocker, new_dir, partitions):
        ...
        p1 = Part(
            "p1",
            {"plugin": "nil", "stage-packages": ["pkg1"]},
            partitions=partitions,
        )
