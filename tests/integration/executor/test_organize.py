...
def test_organize_special_files(new_dir, mocker):
    ...
    os.mkfifo("bar.fifo")
    socket.socket(socket.AF_UNIX, socket.SOCK_STREAM).bind("baz.socket")
    ...
    assert Path("prime/baz.socket").exists() is False
    ...
    assert stat.S_ISSOCK(os.stat("prime/dest/baz.socket").st_mode) != 0
