...
def test_uv_plugin(new_dir, partitions):
    ...
    lf = LifecycleManager(
        parts, application_name="test_uv", cache_dir=new_dir, partitions=partitions
    )
    ...
    assert primed_script.open().readline().rstrip() == "#!/usr/bin/env python"


def test_uv_plugin_with_pyproject_toml(new_dir, partitions):
    """Prime a simple python source."""
    ...
    lf = LifecycleManager(
        parts,
        application_name="test_uv_pyproject_toml",
        cache_dir=new_dir,
...
def test_uv_plugin_symlink(new_dir, partitions):
...
def test_uv_plugin_override_get_system_interpreter(new_dir, partitions):
...
def test_uv_plugin_remove_symlinks(new_dir, partitions):
...
def test_uv_plugin_fix_shebangs(new_dir, partitions):
...
def test_uv_plugin_override_shebangs(new_dir, partitions):
