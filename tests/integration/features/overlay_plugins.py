...
class HostCommandPlugin(Plugin):
    """A plugin that runs host commands on the mounted overlay."""

    properties_class = OverlayPluginProperties
    uses_overlay = True

    def get_overlay_host_commands(self):
        return [f"touch {self._part_info.overlay_mount_dir}/host-proof.txt"]
